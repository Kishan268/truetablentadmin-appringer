...
@section('title', app_name() . ' | Jobs Management')
                        ...
                        <div class="row filter-fix">
                            ...
                            @can('add_gig')
                                <div class="col-md-3 mt-1">
                                    <a href="{{ route('admin.auth.company.job.create') }}">
                    ...
                    <table class="table" id="dataTableExample1">
                        <thead>
                            <tr>
                                ...
                                <th data-name="min_salary" class="sortable">Salary</th>
...
<script>
    $(document).ready(function() {
        ...
        $('#search').on("keypress", function(e) {
            if (e.keyCode == 13) {
                var q = $(this).val();
                var url = "{{ route('admin.auth.company.alljobs') }}";
        ...
        $('body').on('click', '.sortable', function() {
            ...
            if (desc || (!asc && !desc)) {
                getjobs('&column=' + sort_column + '&sort=asc');
                $this.addClass('asc');
            } else {
                getjobs('&column=' + sort_column + '&sort=desc');
                $this.addClass('desc');
            }
        });
        //custom pagination..............
        ...
        function getjobs(params = '', $url = '') {
            $('body').find('.table').addClass('loading1');
            var searchStatus = $('#status').val();
            params += "&status=" + searchStatus;
            var url = $url != '' ? $url : "{{ route('admin.auth.company.jobs-list', ['type' => 'jobs']) }}" +
                params;
            $.ajax({
                type: "get",
                url: url,
        ...
        $('body').on('click', '.sortable', function() {
            ...
            if (desc || (!asc && !desc)) {
                getjobs('&column=' + sort_column + '&sort=asc' + '&from_date=' + from_date +
                    '&to_date=' + to_date);
                $this.addClass('asc');
            } else {
                getjobs('&column=' + sort_column + '&sort=desc' + '&from_date=' + from_date +
                    '&to_date=' + to_date);
                $this.addClass('desc');
            }
        ...
        function dateFilter() {
            ...
            if (from_date && to_date) {
                if (from_date <= to_date) {
                    getjobs('&from_date=' + from_date + '&to_date=' + to_date, '');
                } else {
                    $('#to_date').val('');
        ...
        $('#export').on('click', function() {
            ...
            if (from_date && to_date) {
                ...
                var url = "{{ route('admin.auth.jobs.exports', ['type' => 'export-jobs']) }}" +
                    params;
                $.ajax({
                    type: "post",
                    url: url,
                    ...
                    success: function(response, textStatus, request) {
                        ...
                        link.download = 'jobs-' + from_date + '-to-' + to_date +
                            `.xlsx`;
                        link.click();
