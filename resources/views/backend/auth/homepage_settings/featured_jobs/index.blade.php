                         ...
                         <div class="col-md-3 column rearrange{{$i}} item" id="{{$i}}" >
                            ...
                            <div class="card  mb-4 thumbnail" style="min-height: 190px;width:100% border-radius: 8px;">
                                ...
                                @foreach($jobs AS $key => $job)
                                    ...
                                    @endif
                                    @endforeach
                                    @if($is_job && isset($jobs[$j]))
                                        <h6 class="card-title mb-2 text-muted " style="margin-left: -10px;">
                                            <img src="{{ $jobs[$j]->company_logo ?  App\Helpers\SiteHelper::getObjectUrl($jobs[$j]->company_logo) : '' }}"  style="max-width: 30px;">
                                        </h6>
                                        <div class="card-content">
                                            <h6 class="card-subtitle truncate-1 mt-3">{{ isset($jobs[$j]) ? $jobs[$j]->title : '' }}</h6>
                                            <span class="card-subtitle truncate-1 mt-1" style="color: #9398A1;  font-size: smaller;">{{ isset($jobs[$j]) ? $jobs[$j]->job_types : '' }}</span>
                                            <h6 class="card-subtitle truncate-1 mt-1" style=" font-size: smaller;"> <a href=""><i class="fa fa-map-marker" aria-hidden="true"></i> {{ isset($jobs[$j]) ? $jobs[$j]->job_locations : '' }}</a></h6>
                                            <p class="card-text mt-2 posted-date" style="display: none;">{{ isset($jobs[$j]) ? date("d-m-Y", strtotime($jobs[$j]->created_at)) : '' }}</p>
                                        </div>
                                        <input type="hidden" name="jobId[]" value="{{$jobs[$j]->job_id}}" id="jobId{{$i}}" data-jobid="{{$jobs[$j]->job_id}}">
                                        <input type="hidden" name="joborder[]" value="{{$i}}" id="joborder{{$i}}" data-joborderid="{{$i}}">
                                        
                                    @if($satusUrl !=='rearrange')
                                        @can('delete_featured_job')
                                        <ul class="navbar-nav">
                                                ...
                                                <div class="dropdown-menu p-2 ml-2" aria-labelledby="profileDropdown">
                                                      ...
                                                      <ul class="list-unstyled p-1">
                                                        <li class="dropdown-item py-2">
                                                            <div class="delete-button" jid="{{ $jobs[$j]->id }}">Delete </div>
                                    ...
                                    @can('add_featured_job')
                                        <img src="{{ asset('img/job.png') }}" alt="" class="mx-2 icon" style="max-width: 140px;height: auto;" data-bs-toggle="modal" data-bs-target="#exampleModal"  data-id="{{$i+1}}">
                                         ...
                                         <input type="hidden" name="joborder[]" value="{{$i+1}}" id="joborder{{$i}}" data-joborderid="{{$i}}">
                                     @endcan
                                @endif
                                </div>
                                <div class="card-footer bg-transparent">
                                    <span class="card-text">{{ isset($jobs[$j]) ? substr($jobs[$j]->company_name, 0, 17)."" : '' }}</span>
                                    <span class="card-text test" style="float: right;">{{ isset($jobs[$j]) ? $jobs[$j]->created_at->diffForHumans() : '' }}</span>
                                </div>
                          </div>
                        </div>
                                            ...
                                            <table class="example-table1 table" id="">
                                              <thead>
                                                <tr>
                                                  ...
                                                  <th data-name="title" class="sortable title">Job Title</th>
                ...
                <thead>
                 ...
                 <tbody  id="{{$satusUrl==='rearrange' ? 'list-grid-example':''}}"  >
                    @for($i = 0; $i < 18; $i++)
                        ...
                        @endforeach
                        @if($is_job && isset($jobs[$j]))
                            <tr id="{{$i}}" class="list-view-job-id"> 
                            @if($gridStatus !=='listview' || $satusUrl==='rearrange')
                                <td class=""> <i class="fa fa-bars" aria-hidden="true"></i> </td>
                            @endif
                                <td class="" id="slot{{$i+1}} "> {{$i+1}} </td>
                                <td class="">  {{$jobs[$j]->job_id}}</td>
                                <td class=""> {{ isset($jobs[$j]) ? $jobs[$j]->company_name : '' }}</td>
                                <td class="">  {{ isset($jobs[$j]) ? $jobs[$j]->title : '' }}</td>
                                <td class="">  {{date("d-m-Y", strtotime($jobs[$j]->created_at))}}</td>
                                <td class="">  {{ isset($jobs[$j]) ? $jobs[$j]->company_name : '' }}</td>
                            </tr>
                             <input type="hidden" name="jobId[]" value="{{$jobs[$j]->job_id}}" id="jobId-list-view{{$i}}" data-jobid-list-view="{{$jobs[$j]->job_id}}">
                        @else
                            @if($satusUrl==='rearrange')
                                <tr id="{{$i}}" class="list-view-job-id">
                                @if($gridStatus !=='listview' || $satusUrl==='rearrange')
     ...
     onUpdate: function (/**Event*/evt, /**Event*/originalEvent) {
        ...
        var ids = [];
          $.each(result, function(key,val) {
            var newCompanyLogoId = $('#jobId'+val.id).attr('data-jobid');
        ...
        $.ajax({
          ...
          {
            if(data === 'error'){
                SwalMessage('Job order worng!','error');
            }else if(data === 'success'){
                SwalMessage('Job rearrange successfully!','success');
                location.reload();
                
            }else{
                SwalMessage(data,'error');
            }
 ...
 // list Grid example
  if ($("#list-grid-example").length) {
    ...
    new Sortable(gridExample, {
     ...
     onUpdate: function (/**Event*/evt, /**Event*/originalEvent) {
        var result = $('.list-view-job-id').sortable("serialize");
        var ids = [];
          $.each(result, function(key,val) {
            var newCompanyLogoId = $('#jobId-list-view'+val.id).attr('data-jobid-list-view');
           ids.push(newCompanyLogoId);
        });
          console.log(ids)
        $.ajax({
          ...
          success: function(data)
          {
           SwalMessage('Job rearrange successfully!');
           setTimeout(function() {
            }, 1500);
     ...
     if($('input[name="selected_job"]:checked').val() !=undefined){
        ...
        $.ajax({
           ...
           success:function(resp){
                SwalMessage('Job added successfully!','success');
                setTimeout(function() {
                    location.reload();
                }, 1500);
           }
...
$('.icon').on('click',function(){ 
    let order_id = $(this).attr('data-id');
       getAllFeaturedJobsList(order_id);
});

//custom pagination...................
    function getAllFeaturedJobsList(params='',$url='',order_id) {
        $('body').find('.table').addClass('loading');
        var url = $url != '' ? $url : "{{ route('admin.auth.featured-jobs.all-featuredjobs-list',['type'=>'']) }}"+params;
        $.ajax({
            type: "get",
            url: url,
    ...
    $('body').on('click', '.sortable', function() {
        ...
        if (desc || (!asc && !desc)) {
            getAllFeaturedJobsList('&column='+sort_column+'&sort=asc');
            $this.addClass('asc');
        } else {
            getAllFeaturedJobsList('&column='+sort_column+'&sort=desc');
            $this.addClass('desc');
        }
