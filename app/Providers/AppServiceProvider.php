...
use App\Services\CustomSmsChannel;
...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $whitelistedEmails = explode(",",AppConfig::getWhitelistedEmails());

        if (is_array($whitelistedEmails) && count($whitelistedEmails) > 0 && $whitelistedEmails[0] != '') {
            Mail::alwaysTo($whitelistedEmails);
        }
        ...
        view()->composer('*', function($view){
            $view->with('settings', SystemSettings::first());
        });

        Notification::extend('custom_sms', function ($app) {
