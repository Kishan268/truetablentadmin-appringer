...
class UserConfirmationController extends Controller
{
    ...
    public function sendConfirmationEmail(ManageUserRequest $request, User $user)
    {
        ...
        $user->notify(new UserNeedsConfirmation($user->confirmation_code,$user->first_name));
    ...
    public function confirm(ManageUserRequest $request, User $user)
    {
        ...
        return redirect()->route('admin.auth.user.index',['type' => $type])->withFlashSuccess(__('alerts.backend.users.confirmed'));
    }
    ...
    public function unconfirm(ManageUserRequest $request, User $user)
    {
        ...
        User::where('id',$user->id)->update(['confirmed'=>0]);
        $type = '';
        if ($request->has('type')) {
            $type = $request->type;
        }
        return redirect()->route('admin.auth.user.index',['type' => $type])->withFlashSuccess(__('alerts.backend.users.unconfirmed'));
    }
}
